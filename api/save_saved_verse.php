...
$source_section = isset($input['sourceSection']) ? $input['sourceSection'] : 'saved_verses';
...
try {
    ...
    if (isset($verse['collections']) && is_array($verse['collections'])) {
        ...
        if (count($sectionCollections) > 0) {
            ...
            $delStmt = $pdo->prepare("DELETE FROM user_verse_collections WHERE verse_id = ? AND collection_name IN ($placeholders)");
        ...
        if (count($verse['collections']) > 0) {
            ...
            $insStmt = $pdo->prepare("INSERT IGNORE INTO user_verse_collections (verse_id, collection_name) VALUES (?, ?)");
