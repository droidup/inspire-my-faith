...
import { X, Trash2, PenTool, AlertTriangle } from 'lucide-react';
import { SermonNote } from '../hooks/useSermonNotes';
import FolderDropdown from './shared/FolderDropdown';
import RichTextEditor from './RichTextEditor';

interface SermonNoteEditModalProps {
  isOpen: boolean;
  onClose: () => void;
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl overflow-hidden border border-stone-100 flex flex-col max-h-[95vh] h-full">
        ...
        <div className="flex items-center justify-between p-4 sm:p-6 border-b border-stone-100 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-semibold text-stone-900">Faith Diary</h3>
              <p className="text-xs text-stone-500 font-medium">Local Browser Storage</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {!isDeleting && (
              <button onClick={handleDeleteClick} className="p-2 text-stone-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors" title="Delete Note">
          ...
          <>
            ...
            <div className="px-4 sm:px-6 py-4 border-b border-stone-100 flex items-center justify-end bg-white z-[60] relative">
              <FolderDropdown 
                ...
                selectedFolders={editingNote.collections || []}
                onChange={(folders) => setEditingNote({ ...editingNote, collections: folders })}
                label="Collections"
              />
            </div>
              ...
              <div className="bg-white p-5 rounded-2xl border border-stone-100 shadow-sm flex flex-col flex-1 h-full min-h-[400px]">
                ...
                <div className="flex-1 -mx-5 -mb-5 border-t border-stone-100 flex flex-col">
                  <RichTextEditor
                    ...
                    onChange={(content) => setEditingNote({ ...editingNote, notes: content })}
